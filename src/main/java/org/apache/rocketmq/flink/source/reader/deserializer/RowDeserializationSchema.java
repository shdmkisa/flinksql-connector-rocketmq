...
public class RowDeserializationSchema
    ...
    private void deserializeBytesMessage(BytesMessage message, Collector<RowData> collector) {
        ...
        if("json".equals(type)){
            try {
                Map<String, Object> map = JsonUtil.objectToMap(body);
                GenericRowData genericRowData = buildRowData(map);
                collector.collect(genericRowData);
            }catch (Exception e){
                LOGGER.error("message: "+body+" can not convert to map");
                e.printStackTrace();
            }
        }else {
    ...
    private GenericRowData buildRowData(Map<String, Object> map){
        ...
        for (int index = 0; index < totalColumnSize; index++) {
            ...
            rowData.setField(index,StringSerializer.deserialize(
                    value,
                    fieldTypes[index],
                    fieldDataTypes[index],
                    new HashSet<>()));
    ...
    public static class Builder {
        ...
        public Builder setProperties(Map<String, String> properties) {
            ...
            this.setType(configuration.getString(CollectorOption.TYPE));
    ...
    public static class CollectorOption {
        ...
        public static final ConfigOption<String> TYPE =
                ConfigOptions.key("type".toLowerCase()).defaultValue("NONE");
    }
}
