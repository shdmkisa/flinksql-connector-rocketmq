...
public class RocketMQRowDataConverter implements Serializable {
    ...
    public void open() {
        ...
        bodyFieldNames = new String[rowTypeInfo.getArity() - excludedFields.size()];

        int index = 0;
        for (int num = 0; num < rowTypeInfo.getArity(); num++) {
            if (!excludedFields.contains(num)) {
                bodyFieldIndexes[index] = num;
                bodyFieldNames[index] = rowTypeInfo.getFieldNames()[index];
    ...
    public Message convert(RowData row) {
        ...
        if (onlyVarbinary) {
            ...
            try {
                if("json".equals(type)){
                    Map<String, Object> map = rowData2Map(row, values);
                    message.setBody(JsonUtil.toJson(map).getBytes(encoding));
                }else {
    ...
    //将RowData数据转换为map
    private Map<String,Object> rowData2Map(RowData rowData,Object[] values){
        int arity = rowData.getArity();
        Map<String, Object> map = new LinkedHashMap<>((arity << 2) / 3);
        for (int i = 0; i < arity; i++) {
            String key = bodyFieldNames[i];
