...
public class RocketMQOptions {
    ...
    //数据格式  josn  text(默认，需要配置fieldDelimiter属性)
    ...
    public static final ConfigOption<Boolean> OPTIONAL_USE_NEW_API =
            ConfigOptions.key("useNewApi").booleanType().defaultValue(false);
    ...
    //重启策略：latest  earliest  timestamp  groupOffset
    public static final ConfigOption<String> OPTIONAL_CONSUMER_OFFSET_RESET_POLICY=
            ConfigOptions.key("restPolicy").stringType().defaultValue("groupOffset");

    //重启时间
    public static final ConfigOption<Long> OPTIONAL_CONSUMER_OFFSET_RESET_TIME =
            ConfigOptions.key("timeFrom").longType().defaultValue(System.currentTimeMillis());
}
